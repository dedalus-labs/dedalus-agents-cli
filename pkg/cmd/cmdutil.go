...
func getDefaultRequestOptions(cmd *cli.Command) []option.RequestOption {
	...
	if cmd.IsSet("as-base-url") {
		opts = append(opts, option.WithAsBaseURL(cmd.String("as-base-url")))
	}
	if cmd.IsSet("dedalus-org-id") {
		opts = append(opts, option.WithDedalusOrgID(cmd.String("dedalus-org-id")))
	}
	if cmd.IsSet("provider") {
		opts = append(opts, option.WithProvider(cmd.String("provider")))
	}
	if cmd.IsSet("provider-key") {
		opts = append(opts, option.WithProviderKey(cmd.String("provider-key")))
	}
	if cmd.IsSet("provider-model") {
		opts = append(opts, option.WithProviderModel(cmd.String("provider-model")))
	}
